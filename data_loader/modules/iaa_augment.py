...
class IaaAugment():
    ...
    def may_augment_annotation(self, aug, data, shape):
        ...
        all_poly = [] #store all point
        for poly in data['text_polys']:
            for p in poly:
                all_poly.append(imgaug.Keypoint(p[0], p[1])) #  get all Keypoint
        keypoints = aug.augment_keypoints([imgaug.KeypointsOnImage(all_poly, shape=shape)])[0].keypoints #transfrom all point
        final_poly =  np.array([(p.x, p.y) for p in keypoints]).reshape([-1, 4, 2]) # reshape to boxes
